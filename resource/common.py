...
def validate_token(func):
    ...
    this decorator will place the user's tahoe object under
    the named argument `user_object`.
    ...
    def wrapper(self, req, resp, *args, **kwargs):
        ...
        try:
            ...
            logging.exception("Error in validate_token", exc_info=True)
...
##    '''checks if key `org_hash` is in the named argument `request_data`. If it fails it returns 400. 
##        named argument `request_data` must be passed in to original function
##        or @_extract_request_data() must be called before hand.
##        this decorator will get the `org_hash` from request_data["org_hash"].
##        this decorator will place the orgs tahoe object under the named argument `org_object`.
##    Usage
##    -----
##    @_extract_request_data()
##    @_get_org_object
